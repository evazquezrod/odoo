...
class Many2one(_Relational[M]):
    ...
    def _update_inverses(self, records: BaseModel, value):
        ...
        for invf in records.pool.field_inverses[self]:
            valid_records = records._filtered_access('read').filtered_domain(invf.get_comodel_domain(corecord))
...
class Many2many(_RelationalMulti[M]):
    ...
    def write_real(self, records_commands_list, create=False):
        ...
        old_relation = {record.id: set() if create else set(record[self.name]._ids) for record in records.sudo()}
