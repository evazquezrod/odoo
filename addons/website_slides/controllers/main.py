...
class WebsiteSlides(WebsiteProfile):
    ...
    def slides_channel_all_values(self, slide_category=None, slug_tags=None, my=False, **post):
        ...
        channel_search_count, channel_details, channel_fuzzy_search_term = request.website._search_with_fuzzy(
            "slide_channels_only", search, limit=1000, order=order, options=options)
        channels = channel_details[0].get('results', request.env['slide.channel'])

        # Don't show contents because tags are related to courses
        if options['tag'] is None:
            slide_search_count, slide_details, slide_fuzzy_search_term = request.website._search_with_fuzzy("slides_only",
                channel_fuzzy_search_term or search, limit=1000, order="" if order is None else order, options=options)
            slides = slide_details[0].get('results', request.env['slide.slide'])
            grouped_slides_by_category = slides.grouped('slide_category')
        else:
            grouped_slides_by_category, slide_search_count, slide_fuzzy_search_term = {}, 0, None
        ...
        render_values.update({
            ...
            'search_term': channel_fuzzy_search_term or slide_fuzzy_search_term or search,
            'original_search': (channel_fuzzy_search_term or slide_fuzzy_search_term) and search,
            'search_slide_category': slide_category,
            'search_my': my,
            'search_tags': search_tags,
            'search_count': channel_search_count + slide_search_count,
