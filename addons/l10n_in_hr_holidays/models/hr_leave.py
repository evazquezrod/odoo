...
class HolidaysRequest(models.Model):
    ...
    def _l10n_in_apply_sandwich_rule(self, leave_days, public_holidays, employee_leaves):
        ...
        def count_adjacent_non_working_days(start_date, direction=1, include_start=False):
            current = start_date if include_start else start_date + timedelta(days=direction)
            count = 0
        ...
        user_timezone = pytz.timezone(self.env.context.get('tz') or self.env.user.tz or 'UTC')
        ...
        if linked_before:
            total_leaves += count_adjacent_non_working_days(date_from, direction=-1)
        ...
        if linked_after:
            ...
            total_leaves -= count_adjacent_non_working_days(date_to, direction=-1, include_start=True)
