...
class TestTrackingInternals(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.records = cls.env['mail.test.ticket'].create([{'name': f'Record {i}'} for i in range(3)])
        cls.properties_parent_1, cls.properties_parent_2 = cls.env['mail.test.track.all.properties.parent'].create([{
            'definition_properties': [
                ...
                {'name': 'property_m2o', 'string': 'Property M2O', 'type': 'many2one',
                 'default': (cls.records[0].id, cls.records[0].display_name), 'comodel': 'mail.test.ticket'},
            ],
        }, {
            'definition_properties': [
                {'name': 'property_m2m', 'string': 'Property M2M', 'type': 'many2many',
                 'default': [(rec.id, rec.display_name) for rec in cls.records], 'comodel': 'mail.test.ticket'},
    ...
    def test_mail_track_properties(self):
        ...
        record.properties_parent_id = self.properties_parent_2
        self.flush_tracking()
        self.assertFalse(record.mapped("message_ids.tracking_value_ids"))
