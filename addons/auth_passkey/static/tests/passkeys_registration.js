...
import { PassKeyNameFormController } from "../src/views/auth_passkey_key_create_form_view";
    ...
    steps: () => [
        {
            ...
            run: () => {
                // !! REVIEW !!
                // This is an ugly hack but I'm not sure of a better way of handling it.
                // Mocking navigator.credentials.create has many problems, including having to
                // create an entire wrapper for PublicKeyCredential, which clogs up the code.
                PassKeyNameFormController.prototype.PASSKEY_TEST_REGISTRATION = {
                    // !! REVIEW !!
                    // test-yubikey hardcoded because there didn't seem to be a simply way of getting this info unless I made a new route, which seems overkill
            ...
            trigger: 'body',
            run: () => {
                PassKeyNameFormController.prototype.PASSKEY_TEST_REGISTRATION = null;
            }
        }, {
            content: 'Open user account menu',
            trigger: '.o_user_menu .dropdown-toggle',
