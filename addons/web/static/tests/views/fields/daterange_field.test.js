...
test("Datetime field - interaction with the datepicker", async () => {
    ...
    expect("button[data-field=datetime]").toHaveValue("02/08/2017 15:30");
    expect("button[data-field=datetime_end]").toHaveValue("02/09/2017 05:30");
});
...
test("Datetime field - interaction with the datepicker (same initial dates)", async () => {
    ...
    expect("button[data-field=datetime]").toHaveValue("02/08/2017 15:30");
    expect("button[data-field=datetime_end]").toHaveValue("02/08/2017 20:30");
    ...
    await animationFrame();
    expect("input[data-field=datetime]").toHaveValue("02/08/2017 15:30");
    expect("button[data-field=datetime_end]").toHaveValue("02/10/2017 20:30");
});
...
test("Datetime field manually input value should send utc value to server", async () => {
    ...
    expect("button[data-field=datetime]").toHaveValue("02/08/2017 15:30");
    expect("button[data-field=datetime_end]").toHaveValue("03/13/2017 05:30");
    ...
    expect("button[data-field=datetime]").toHaveValue("02/08/2017 11:30");
});
...
test("Render with initial empty value: datetime field", async () => {
    ...
    expect(".o_field_daterange input:eq(0)").toHaveValue("08/14/2014 12:00");
    ...
    expect("input[data-field=datetime]").toHaveValue("08/14/2014 12:00");
    expect("button[data-field=datetime_end]").toHaveValue("08/14/2014 13:00");
});
...
test("Render with initial empty value and optional start date", async () => {
    ...
    // Select a value (today)
    await contains(".o_today").click();
    expect(".o_field_daterange input:eq(0)").toHaveValue("08/14/2014 13:00");
    expect(".o_add_date").toBeVisible();
    ...
    expect("button[data-field=datetime]").toHaveValue("08/14/2014 12:00");
    expect("input[data-field=datetime_end]").toHaveValue("08/14/2014 13:00");
});
...
test("initial empty date with optional start date", async () => {
    ...
    contains("button[data-field=datetime]").focus();
    await animationFrame();
    expect(".o_add_date").toBeVisible();
    expect(".o_datetime_picker").toHaveCount(0);
    expect(".o_add_date").toHaveText("Add end date");
    ...
    expect("input[data-field=datetime]").toHaveValue("03/13/2017 00:00");
    expect("button[data-field=datetime_end]").toHaveValue("03/13/2017 01:00");
});
...
test("initial empty date with optional end date", async () => {
    ...
    expect(".o_add_date").not.toBeVisible();
    await contains("button[data-field=datetime_end]").focus();
    expect(".o_add_date").toBeVisible();
    expect(".o_add_date").toHaveText("Add start date");
    ...
    expect("button[data-field=datetime]").toHaveValue("03/12/2017 23:00");
    expect("input[data-field=datetime_end]").toHaveValue("03/13/2017 00:00");
});
...
test("related end date, both start date and end date empty", async () => {
    ...
    expect(".o_field_daterange input").toHaveCount(1);
    expect(".o_field_daterange input:eq(0)").toHaveAttribute("data-field", "datetime");
    expect(".o_field_daterange input:eq(0)").toHaveValue("06/06/2023 12:00");
    expect(".o_add_date").toHaveText("Add end date");
    await contains(".o_add_date:enabled").click();
    expect(".o_field_daterange input").toHaveCount(1);
    expect(".o_field_daterange input").toHaveAttribute("data-field", "datetime");
    expect(".o_field_daterange input").toHaveValue("06/06/2023 12:00");
    expect(".o_field_daterange button").toHaveAttribute("data-field", "datetime_end");
    expect(".o_field_daterange button").toHaveValue("06/06/2023 13:00");
    expect(".o_add_date").toHaveCount(0);
});
...
test("required: related end date, both start date and end date empty", async () => {
    ...
    expect(".o_field_daterange button:eq(0)").toHaveValue("06/06/2023 12:00");
    expect(".o_field_daterange button:eq(1)").toHaveAttribute("data-field", "datetime_end");
    expect(".o_field_daterange button:eq(1)").toHaveValue("07/07/2023 13:00");
    expect(".o_add_date").toHaveCount(0);
    ...
    expect(".o_field_daterange button").toHaveAttribute("data-field", "datetime_end");
    expect(".o_field_daterange button").toHaveValue("07/07/2023 13:00");
    expect(".o_add_date").toHaveCount(0);
...
test("related start date, both start date and end date set", async () => {
    ...
    expect(".o_field_daterange button").toHaveCount(2);
    expect(".o_field_daterange button:eq(0)").toHaveAttribute("data-field", "datetime");
    expect(".o_field_daterange button:eq(0)").toHaveValue("02/08/2017 15:30");
    expect(".o_field_daterange button:eq(1)").toHaveAttribute("data-field", "datetime_end");
    expect(".o_field_daterange button:eq(1)").toHaveValue("02/08/2017 15:30");
    expect(".o_add_date").toHaveCount(0);
    ...
    expect(".o_field_daterange button[data-field]").toHaveValue("02/08/2017 15:30");
    expect(ensureCSSVisible(".o_add_date")).toHaveText("Add start date");
...
test("list daterange: column widths (fancy format)", async () => {
    ...
    await mountView({
        ...
        arch: /* xml */ `
            <list>
                <field name="date" widget="daterange" options="{'end_date_field': 'date_end', 'date_format': true}" />
                <field name="datetime" widget="daterange" options="{'end_date_field': 'datetime_end', 'date_format': true}" />
                <field name="char_field" />
            </list>`,
    });
...
test.tags("desktop");
test.todo("list daterange: start date input width matches its span counterpart", async () => {
    Partner._records[0].datetime_end = "2017-02-09 17:00:00";
    ...
    expect(".o_field_daterange button").toHaveProperty("offsetWidth", initialWidth);
});
...
test("date values are selected eagerly and do not flicker", async () => {
    ...
    expect(".o_field_datetime input").toHaveValue("02/19/2017 15:30");
    expect(".o_field_datetime button").toHaveValue("02/19/2017 16:30");
    expect.verifySteps([]);
...
test("daterange with inverted start date and end date", async () => {
    ...
    expect(".o_field_daterange button:eq(0)").toHaveValue("02/08/2017 15:30");
    expect(".o_field_daterange button:eq(1)").toHaveValue("02/01/2017 05:30");
...
test("updating time keeps selected dates", async () => {
    ...
    expect("button[data-field=datetime]").toHaveValue("02/08/2017 15:30");
    expect("button[data-field=datetime_end]").toHaveValue("03/13/2017 05:32");
    ...
    await animationFrame();
    await contains(".o_time_picker:eq(1) .o_time_picker_input").click();
    await animationFrame();
    await edit("5:05", { confirm: "enter" });
    await animationFrame();

    expect("input[data-field=datetime]").toHaveValue("02/08/2017 15:30");
    expect("button[data-field=datetime_end]").toHaveValue("03/16/2017 05:05");
    expect(".o_time_picker:first .o_time_picker_input").toHaveValue("15:30");
    ...
    expect("input[data-field=datetime]").toHaveValue("02/08/2017 15:35");
    expect("button[data-field=datetime_end]").toHaveValue("03/16/2017 05:05");
