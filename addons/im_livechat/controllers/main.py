...
class LivechatController(http.Controller):
    ...
    def get_session(self, channel_id, previous_operator_id=None, chatbot_script_id=None, persisted=True):
        ...
        bot = request.env["chatbot.script"]
        ...
        if not persisted:
            ...
            chatbot_data = None
            if bot:
                welcome_steps = bot._get_welcome_steps()
                chatbot_data = {
                    "script": bot.id,
            ...
            operator_partner = agent.partner_id if agent else bot.operator_partner_id
            channel_info = {
            ...
            if bot:
                bot._post_welcome_steps(channel)
            if not bot or bot.operator_partner_id != channel.livechat_operator_id:
