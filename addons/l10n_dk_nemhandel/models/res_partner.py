...
class ResPartner(models.Model):
    ...
    nemhandel_verification_state = fields.Selection(
        selection=[
            ...
            ('not_valid_format', 'Cannot receive this format'),  # registered on Nemhandel but cannot receive the selected document type
            ('valid', 'Valid'),
        ],
    ...
    def button_nemhandel_check_partner_endpoint(self, company=None):
        ...
        self_partner.nemhandel_verification_state = self._get_nemhandel_verification_state(
            self.nemhandel_identifier_type,
            self.nemhandel_identifier_value,
    ...
    @api.model
    @handle_demo
    def _get_nemhandel_verification_state(self, nemhandel_identifier_type, nemhandel_identifier_value, invoice_edi_format):
        if not nemhandel_identifier_type or not nemhandel_identifier_value or invoice_edi_format != 'oioubl_21':
            return 'not_verified'

        edi_identification = f"{nemhandel_identifier_type}:{nemhandel_identifier_value}".lower()
        ...
        if participant_info is None:
            ...
            if is_participant_on_network:
                is_valid_format = self._check_document_type_support(participant_info, 'oioubl_21')
                if is_valid_format:
                    return 'valid'
                else:
