...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_alerts(self, moves, moves_data):
        # EXTENDS 'account'
        def nemhandel_partner(moves):
            return moves.partner_id.commercial_partner_id

        def filter_nemhandel_state(moves, states):
            return nemhandel_partner(moves.filtered(
                lambda m: self.env['res.partner']._get_nemhandel_verification_state(
                    nemhandel_partner(m).nemhandel_identifier_type,
                    nemhandel_partner(m).nemhandel_identifier_value,
                    moves_data[m]['invoice_edi_format']) in states)
            )

        alerts = super()._get_alerts(moves, moves_data)
        # Check for invalid nemhandel partners.
        nemhandel_moves = moves.filtered(lambda m: 'nemhandel' in moves_data[m]['sending_methods'])
        invalid_partners = filter_nemhandel_state(nemhandel_moves, ['not_valid_format'])
        if invalid_partners and not 'account_edi_ubl_cii_configure_partner' in alerts:
            alerts['account_nemhandel_warning_partner'] = {
                'message': _("Customer is on Nemhandel but did not enable receiving documents."),
    ...
    def _is_applicable_to_move(self, method, move, **move_data):
        ...
        if method == 'nemhandel':
            ...
            return all([
                ...
                self.env['res.partner']._get_nemhandel_verification_state(partner.nemhandel_identifier_type, partner.nemhandel_identifier_value, invoice_edi_format) == 'valid',
                move.company_id.l10n_dk_nemhandel_proxy_state != 'rejected',
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            ...
            if 'nemhandel' in invoice_data['sending_methods']:
                ...
                if self.env['res.partner']._get_nemhandel_verification_state(partner.nemhandel_identifier_type, partner.nemhandel_identifier_value, invoice_data['invoice_edi_format']) != 'valid':
                    invoice.nemhandel_move_state = 'error'
