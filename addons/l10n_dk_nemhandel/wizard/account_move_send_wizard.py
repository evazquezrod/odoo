...
class AccountMoveSendWizard(models.TransientModel):
    ...
    def _compute_sending_method_checkboxes(self):
        ...
        for wizard in self:
            if nemhandel_checkbox := wizard.sending_method_checkboxes.get('nemhandel'):
                nemhandel_partner = wizard.move_id.partner_id.commercial_partner_id.with_company(wizard.company_id)
                    ...
                    wizard.sending_method_checkboxes = {
                        **wizard.sending_method_checkboxes,
                        'nemhandel': {
                            ...
                            **vals_not_valid,
                            'label': _(
                                '%(nemhandel_label)s%(disable_reason)s%(nemhandel_proxy_mode)s',
                                nemhandel_label=nemhandel_checkbox['label'],
                                disable_reason=addendum_disable_reason,
                                nemhandel_proxy_mode=addendum_mode,
                            ),
                        }
                    }
