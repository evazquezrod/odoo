...
class ProjectTaskBurndownChartReport(models.AbstractModel):
    ...
    @api.model
    def _search(self, domain, offset=0, limit=None, order=None, *, no_record_rules=False):
        burndown_specific_domain, task_specific_domain = self._determine_domains(domain)
        main_query = super()._search(
            burndown_specific_domain, offset=offset, limit=limit, order=order, no_record_rules=True)
        ...
        project_task_query = self.env['project.task']._search(task_specific_domain, no_record_rules=True)
