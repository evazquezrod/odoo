...
test("opens dropdown when typing /", async () => {
    ...
    addOption({
        ...
        template: xml`<BuilderMultiUrlPicker dataAttributeAction="'url'"/>`,
    });
...
test("adding multiple URLs", async () => {
    ...
    await contains(document.querySelector(".o_website_ui_autocomplete > li:nth-child(1) a")).click();

    // Add second URL
    await contains(inputSelector).edit("/page2");
    await contains(inputSelector).click();
    await contains(document.querySelector(".o_website_ui_autocomplete > li:nth-child(2) a")).click();
...
test("can remove a URL", async () => {
    ...
    await expect(":iframe .test-options-target").toHaveAttribute("data-urls", JSON.stringify(["/page2"]));
});
...
test("previously selected URLs", async () => {
    ...
    await setupWebsiteBuilder(`<div class="test-options-target" data-urls='["/page1", "/page2"]'>b</div>`);
    await contains(":iframe .test-options-target").click();
