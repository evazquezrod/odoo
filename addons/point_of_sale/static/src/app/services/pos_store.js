...
export class PosStore extends WithLazyGetterTrap {
    ...
    async openConfigurator(
        pTemplate,
        opts = {},
        { defaultAttributeIds, defaultCustomAttributeIds } = {}
    ) {
     ...
     * This actions cannot be handled inside pos_order.js or pos_order_line.js
     */
    async handleComboProduct(values, order, configure = true, { defaultComboLineIds } = {}) {
     ...
     */
    getCashier() {
        this.user._role = this.user._raw.role;
        return this.user;
    }
    getCashierUserId() {
        return this.user.id;
    ...
    async syncAllOrders(options = {}) {
        ...
        try {
            ...
            if (data["pos.session"].length > 0) {
                ...
                const sessions = this.models["pos.session"].getAll().sort((a, b) => a.id - b.id);
