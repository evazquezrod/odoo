...
from odoo.addons.hr_holidays.models.hr_leave_accrual_plan import get_lvl_start_date
...
class HrLeaveAllocation(models.Model):
    ...
    last_accruals = fields.Date("Date of the last accrual allocation", readonly=False)
    # actual_lastcall stores the date of the last executed accrual event (accruals, carryover_date; ...)
    actual_lastcall = fields.Date(export_string_translation=False, readonly=False)
    # nextcall stores the dates of the closest next accrual event (accruals, carryover_date, ...)
    nextcall = fields.Date("Date of the next accrual allocation", readonly=False, default=False)
    ...
    def _add_days_to_allocation(self, current_level, current_level_maximum_leave, leaves_taken, period_start, period_end, start_date, end_date):
        self.ensure_one()
    ...
    def _get_current_accrual_plan_level_id(self, date, lvl_intervals=False):
        self.ensure_one()
        if not self.accrual_plan_id:
            return (False, False)
        accrual_plan_level = self.accrual_plan_id._get_current_accrual_plan_level_id(self.date_from, date, lvl_intervals)
        if not accrual_plan_level:
            return (False, False)
        if len(accrual_plan_level) == 2 and self.accrual_plan_id.accrued_gain_time == 'start':
            # Select the second level as the next accrual event will be the accrual of the next period of the next level
            return accrual_plan_level[1]
        return accrual_plan_level[0]
    ...
    def _get_leaves_taken_days(self):
        self.ensure_one()
        if self.holiday_status_id.request_unit in ["day", "half_day"]:
            return self.leaves_taken
        return self.leaves_taken / self.employee_id._get_hours_per_day(self.date_from)

    def _get_lvl_max_leave_days(self, level):
        self.ensure_one()
        if level.added_value_type == "day":
            return level.maximum_leave
        return level.maximum_leave / self.employee_id._get_hours_per_day(self.date_from)

    def _update_carryover_date(self, carryover_date, current_level):
        self.ensure_one()
        self.carried_over_days_expiration_date = carryover_date + \
    ...
    def _process_expiration_date(self, current_level, leaves_taken, carryover_date):
        ...
        if current_level.accrual_validity:
            self._update_carryover_date(carryover_date, current_level)
        ...
        if current_level.action_with_unused_accruals in ['lost', 'maximum']:
            ...
            if current_level.action_with_unused_accruals == 'maximum':
                if current_level.added_value_type == 'day':
                    ...
                    postpone_max_days = current_level.postpone_max_days / self.employee_id._get_hours_per_day(self.date_from)
    ...
    def _process_accrual_plans(self, date_to=False, force_period=False, log=True):
        ...
        for allocation in self:
            if allocation.accrual_plan_id.accrued_gain_time == 'start':
                self._process_accrual_plans_start(allocation, date_to, force_period, log, first_allocation)
            else:
                self._process_accrual_plans_end(allocation, date_to, force_period, log, first_allocation)

    def _process_accrual_plans_start(self, allocation, date_to, force_period, log, first_allocation):
        ...
        first_level_start_date = get_lvl_start_date(allocation.date_from, first_level)

        if date_to < first_level_start_date:
            return

        if not allocation.nextcall:
            # First time the plan is run, init it
            allocation.last_accruals = False
            allocation.actual_lastcall = first_level_start_date
            allocation.nextcall = allocation.actual_lastcall
            if log:
                allocation._message_log(body=first_allocation)

        current_level_maximum_leave = False
        lvl_intervals = allocation.accrual_plan_id.get_lvls_intervals(allocation.date_from, level_ids)
        while allocation.nextcall <= date_to:
            current_level, current_level_idx = allocation._get_current_accrual_plan_level_id(allocation.nextcall, lvl_intervals)
            if not current_level:
                break

            if current_level.cap_accrued_time:
                current_level_maximum_leave = allocation._get_lvl_max_leave_days(current_level)

            period_end = current_level._get_next_date(allocation.nextcall)
            period_start = current_level._get_previous_date(allocation.nextcall)
            nextcall = period_end
            ...
            current_level_last_date = allocation.accrual_plan_id.get_lvl_last_date(allocation.date_from, current_level_idx, lvl_intervals)
            if current_level_last_date and allocation.nextcall != current_level_last_date:
                nextcall = min(nextcall, current_level_last_date)

            no_carryover_nextcall = nextcall
            ...
            if current_level.accrual_validity and allocation.carried_over_days_expiration_date and \
                allocation.nextcall < allocation.carried_over_days_expiration_date < nextcall:
                nextcall = allocation.carried_over_days_expiration_date

            if current_level.accrual_validity and allocation.nextcall == allocation.carried_over_days_expiration_date:
                allocation._process_expiration_date(current_level, leaves_taken, carryover_date)

            current_lvl_start_date = get_lvl_start_date(allocation.date_from, current_level)
            is_accrual_date = allocation.nextcall == period_start or allocation.nextcall == current_level_last_date or current_lvl_start_date == allocation.nextcall
            if is_accrual_date:
                allocation._add_days_to_allocation(current_level, current_level_maximum_leave, leaves_taken, period_start, period_end, allocation.nextcall, no_carryover_nextcall)

            if allocation.nextcall == carryover_date:
                allocation.yearly_accrued_amount = 0

            if is_accrual_date:
                allocation.last_accruals = allocation.nextcall
    ...
    def _process_accrual_plans_end(self, allocation, date_to, force_period, log, first_allocation):
        level_ids = allocation.accrual_plan_id.level_ids.sorted('sequence')
        if not level_ids:
            return
        ...
        if not allocation.nextcall:
            allocation.last_accruals = False
            allocation.actual_lastcall = first_level_start_date
            allocation.nextcall = first_level._get_next_date(allocation.actual_lastcall)

            carryover_date = allocation._get_carryover_date(allocation.nextcall)
            allocation.nextcall = min(carryover_date, allocation.nextcall)

            if len(level_ids) > 1:
                second_level_start_date = allocation.accrual_plan_id.get_lvl_last_date(allocation.date_from, 0)
        ...
        while allocation.nextcall <= date_to:
            ...
            period_end = current_level._get_next_date(allocation.actual_lastcall)
            ...
            if is_accrual_date:
                allocation._add_days_to_allocation(current_level, current_level_maximum_leave, leaves_taken, period_start, period_end, allocation.last_accruals or first_level_start_date, allocation.nextcall)
    ...
    # it uses the actual computation function but resets values of last_accruals, nextcall and nbr of days
    ...
    def _onchange_date_from(self):
        if not self.date_from or self.allocation_type != 'accrual' or self.state == 'validate' or not self.accrual_plan_id\
           or not self.employee_id:
            return
        self.last_accruals = self.date_from
        self.actual_lastcall = self.last_accruals
        self.nextcall = False
