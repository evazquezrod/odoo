...
"""
The :mod:`odoo.addons.base_paper_muncher.engine.paper_muncher` module provides
...
from odoo.addons.base_paper_muncher.engine.utils.odoo_request import (
    generate_odoo_http_response
)
from odoo.addons.base_paper_muncher.engine.utils.binary import (
    get_paper_muncher_binary,
    can_use_paper_muncher,
)
from odoo.addons.base_paper_muncher.engine.utils.pm_request import (
    consume_paper_muncher_request,
    read_paper_muncher_request,
)
from odoo.addons.base_paper_muncher.engine.utils.io_with_timeout import (
    read_all_with_timeout,
    write_with_timeout,
)
from odoo.addons.base_paper_muncher.engine.utils.popen import Popen
    ...
    **options,
    ...
    with Popen(
        ...
        for request_no in count(start=1):
            try:
                path = read_paper_muncher_request(
                    process.stdout, 
                    timeout=readline_timeout,
                )
    ...
    **options,
    ...
    with rendered(
        ...
        **options,
        ...
        rendered_content = read_all_with_timeout(
            content_stream,
            chunk_size=chunk_size,
            timeout=read_timeout,
        )
        stderr_output = read_all_with_timeout(
            error_stream,
            chunk_size=chunk_size,
            timeout=read_timeout,
        )
