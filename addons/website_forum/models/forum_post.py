...
class ForumPost(models.Model):
    ...
    def _get_current_partner(self):
        ...
        return self.env['res.partner'].browse(request and request.session.get('partner_id'))
    ...
    def _compute_user_favourite(self):
        for post in self:
            post.user_favourite = post._uid in post.sudo().favourite_ids.ids
    ...
    def _search_user_favourite(self, operator, value):
        ...
        domain = [('favourite_ids', '=' if value else '!=', self.env.user.id)]
        ...
        if 'content' in vals or 'name' in vals:
            for post in self:
                if post.parent_id:
                    ...
                    partner_ids = post.sudo().follower_ids | post.sudo().parent_id.follower_ids
                else:
                    body, subtype_xmlid = _('Question Edited'), 'website_forum.mt_question_edit'
                    partner_ids = post.sudo().follower_ids
    ...
    def _search_get_detail(self, website, order, options):
        ...
        if my == 'followed':
            ...
            domain = expression.AND([domain, [('user_favourite', '=', True)]])
        elif my == 'upvoted':
