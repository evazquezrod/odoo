...
class AccountMove(models.Model):
    ...
    def _compute_l10n_my_edi_state(self):
        for move in self:
            myinvois_document = move.l10n_my_edi_document_ids._get_active_document(including_in_progress=True)
    ...
    def button_request_cancel(self):
        ...
        active_myinvois_document = self._get_active_document()
    ...
    def _need_cancel_request(self):
        ...
        return super()._need_cancel_request() or self._get_active_document()

    def _get_name_invoice_report(self):
        # EXTENDS 'account'
        # Meaning we are a myinvois invoice, meaning we need to embed the qr code.
        if self._get_active_document():
    ...
    def action_l10n_my_edi_update_status(self):
        self.ensure_one()
        self.l10n_my_edi_document_ids._get_active_document(including_in_progress=True).action_update_submission_status()
    ...
    def action_show_myinvois_documents(self):
        if len(self.l10n_my_edi_document_ids) == 1:
            action_vals = {
                'type': 'ir.actions.act_window',
                'res_model': 'myinvois.document',
                'view_mode': 'form',
                'res_id': self.l10n_my_edi_document_ids.id,
            ...
            action_vals = {
                ...
                'domain': [('id', 'in', self.l10n_my_edi_document_ids.ids)],
            }
        return action_vals
    ...
    def _generate_myinvois_qr_code(self):
        ...
        myinvois_document = self._get_active_document()
    ...
    def _get_active_document(self):
        # todo add the in progress thing and update name (same of the others) for conflict
        """ Shortcut to get the active document of all invoice in self. """
        return self.env['myinvois.document'].union(*[invoice.l10n_my_edi_document_ids._get_active_document() for invoice in self])
