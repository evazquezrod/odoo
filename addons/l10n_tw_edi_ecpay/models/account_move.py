...
class AccountMove(models.Model):
    ...
    def _l10n_tw_edi_to_integer(self, num):
        """
        Round the number to integer using ROUND_HALF_UP (3.15 -> 3, 3.55 -> 4)
        """
    ...
    def _l10n_tw_edi_prepare_item_list(self, json_data, is_allowance=False):
        ...
        difference = self._l10n_tw_edi_to_integer(sale_amount - abs(amount_on_invoice))
        if difference != 0:
            item_list[-1]["ItemAmount"] -= difference
            item_list[-1]["ItemPrice"] = item_list[-1]["ItemAmount"] / item_list[-1]["ItemCount"]
        ...
        if not is_allowance:
            json_data["SalesAmount"] = self._l10n_tw_edi_to_integer(sale_amount)
        else:
            if self.l10n_tw_edi_is_b2b:
                json_data["TotalAmount"] = self._l10n_tw_edi_to_integer(sale_amount)
            else:
                json_data["AllowanceAmount"] = self._l10n_tw_edi_to_integer(sale_amount)

        if self.l10n_tw_edi_is_b2b:
            json_data["TaxAmount"] = self._l10n_tw_edi_to_integer(tax_amount) if tax_type != "4" else 0
