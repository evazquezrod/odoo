...
def git(*args):
    ...
    """
    git_executable = 'git' if IS_IOT_BOX else path_file('git', 'cmd', 'git.exe')
...
def pip(*args):
    ...
    """
    python_executable = [] if IS_IOT_BOX else [path_file('python', 'python.exe'), '-m']
    command = [*python_executable, 'pip', *args]

    if IS_IOT_BOX and args[0] == 'install':
        command.append('--user')
