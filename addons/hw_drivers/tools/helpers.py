...
from odoo.addons.hw_drivers.tools.iot_system import IOT_SYSTEM, IS_IOT_TEST, IS_IOT_BOX, IS_WINDOWS, IS_TESTING
...
def get_ip():
    if IS_TESTING:
        # mandatory in test mode due to `fetch_proxy` preventing calls from LAN IP addresses
        return '127.0.0.1'
...
def load_iot_handlers():
    ...
    for directory in ['interfaces', 'drivers']:
        ...
        handler_files = list_file_by_iot_system(path)
...
def list_file_by_iot_system(handler_path):
    """
    List all handler files that an IoT system should load.
    - IoT boxes load file without suffixe and _L
    - Windows IoT load file without suffixes and _W
    - Test IoT only load files with _T suffix if any
    Note: this function will be patched in odoo's tests to force load certain handlers
    ...
    :rtype list:
    """
    suffix_to_ignore = tuple(set(IOT_SYSTEM.get_all_value()) - {IOT_SYSTEM.value})
    iot_system_files = []
    for filename in Path(handler_path).glob('*.py'):
