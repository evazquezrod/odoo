...
class HrEmployee(models.Model):
    ...
    hr.employee.public model) should have `groups="hr.group_hr_employee_manager"` on its
    ...
    version_id = fields.Many2one(
        ...
        store=False,
        compute_sudo=True,
        groups="hr.group_hr_employee_manager")
    ...
    versions_count = fields.Integer(compute='_compute_versions_count', groups="hr.group_hr_employee_manager")
    ...
    company_country_id = fields.Many2one('res.country', 'Company Country', related='company_id.country_id', readonly=True, groups="base.group_system,hr.group_hr_employee_manager")
    company_country_code = fields.Char(related='company_country_id.code', depends=['company_country_id'], readonly=True, groups="base.group_system,hr.group_hr_employee_manager", string='Company Country Code')
    ...
    # private info
    legal_name = fields.Char(compute='_compute_legal_name', store=True, readonly=False, groups="hr.group_hr_employee_manager")
    private_phone = fields.Char(string="Private Phone", groups="hr.group_hr_employee_manager")
    private_email = fields.Char(string="Private Email", groups="hr.group_hr_employee_manager")
    lang = fields.Selection(selection=_lang_get, string="Lang", groups="hr.group_hr_employee_manager")
    place_of_birth = fields.Char('Place of Birth', groups="hr.group_hr_employee_manager", tracking=True)
    country_of_birth = fields.Many2one('res.country', string="Country of Birth", groups="hr.group_hr_employee_manager", tracking=True)
    birthday = fields.Date('Birthday', groups="hr.group_hr_employee_manager", tracking=True)
    birthday_public_display = fields.Boolean('Show to all employees', groups="hr.group_hr_employee_manager", default=False)
    ...
    permit_no = fields.Char('Work Permit No', groups="hr.group_hr_employee_manager", tracking=True)
    visa_no = fields.Char('Visa No', groups="hr.group_hr_employee_manager", tracking=True)
    visa_expire = fields.Date('Visa Expiration Date', groups="hr.group_hr_employee_manager", tracking=True)
    work_permit_expiration_date = fields.Date('Work Permit Expiration Date', groups="hr.group_hr_employee_manager", tracking=True)
    has_work_permit = fields.Binary(string="Work Permit", groups="hr.group_hr_employee_manager")
    work_permit_scheduled_activity = fields.Boolean(default=False, groups="hr.group_hr_employee_manager")
    work_permit_name = fields.Char('work_permit_name', compute='_compute_work_permit_name', groups="hr.group_hr_employee_manager")
    additional_note = fields.Text(string='Additional Note', groups="hr.group_hr_employee_manager", tracking=True)
    certificate = fields.Selection([
        ...
        ('doctor', 'Doctor'),
        ('other', 'Other'),
    ], 'Certificate Level', groups="hr.group_hr_employee_manager", tracking=True)
    study_field = fields.Char("Field of Study", groups="hr.group_hr_employee_manager", tracking=True)
    study_school = fields.Char("School", groups="hr.group_hr_employee_manager", tracking=True)
    emergency_contact = fields.Char(groups="hr.group_hr_employee_manager", tracking=True)
    emergency_phone = fields.Char(groups="hr.group_hr_employee_manager", tracking=True)
    ...
    category_ids = fields.Many2many(
        ...
        'employee_id', 'category_id', groups="hr.group_hr_employee_manager",
        string='Tags')
    # misc
    color = fields.Integer('Color Index', default=0)
    barcode = fields.Char(string="Badge ID", help="ID used for employee identification.", groups="hr.group_hr_employee_manager", copy=False)
    pin = fields.Char(string="PIN", groups="hr.group_hr_employee_manager", copy=False,
        help="PIN used to Check In/Out in the Kiosk Mode of the Attendance application (if enabled in Configuration) and to change the cashier in the Point of Sale application.")
    message_main_attachment_id = fields.Many2one(groups="hr.group_hr_employee_manager")
    id_card = fields.Binary(string="ID Card Copy", groups="hr.group_hr_employee_manager")
    driving_license = fields.Binary(string="Driving License", groups="hr.group_hr_employee_manager")
    private_car_plate = fields.Char(groups="hr.group_hr_employee_manager", help="If you have more than one car, just separate the plates by a space.")
    currency_id = fields.Many2one('res.currency', related='company_id.currency_id', readonly=True, groups="hr.group_hr_employee_manager")
    related_partners_count = fields.Integer(compute="_compute_related_partners_count", groups="hr.group_hr_employee_manager")
    # properties
    employee_properties = fields.Properties('Properties', definition='company_id.employee_properties_definition', precompute=False, groups="hr.group_hr_employee_manager")

    # mail.activity.mixin
    activity_ids = fields.One2many(groups="hr.group_hr_employee_manager")
    activity_state = fields.Selection(groups="hr.group_hr_employee_manager")
    activity_user_id = fields.Many2one(groups="hr.group_hr_employee_manager")
    activity_type_id = fields.Many2one(groups="hr.group_hr_employee_manager")
    activity_type_icon = fields.Char(groups="hr.group_hr_employee_manager")
    activity_date_deadline = fields.Date(groups="hr.group_hr_employee_manager")
    my_activity_date_deadline = fields.Date(groups="hr.group_hr_employee_manager")
    activity_summary = fields.Char(groups="hr.group_hr_employee_manager")
    activity_exception_decoration = fields.Selection(groups="hr.group_hr_employee_manager")
    activity_exception_icon = fields.Char(groups="hr.group_hr_employee_manager")

    # mail.thread mixin
    message_is_follower = fields.Boolean(groups="hr.group_hr_employee_manager")
    message_follower_ids = fields.One2many(groups="hr.group_hr_employee_manager")
    message_partner_ids = fields.Many2many(groups="hr.group_hr_employee_manager")
    message_ids = fields.One2many(groups="hr.group_hr_employee_manager")
    has_message = fields.Boolean(groups="hr.group_hr_employee_manager")
    message_needaction = fields.Boolean(groups="hr.group_hr_employee_manager")
    message_needaction_counter = fields.Integer(groups="hr.group_hr_employee_manager")
    message_has_error = fields.Boolean(groups="hr.group_hr_employee_manager")
    message_has_error_counter = fields.Integer(groups="hr.group_hr_employee_manager")
    message_attachment_count = fields.Integer(groups="hr.group_hr_employee_manager")
    ...
    def check_field_access_rights(self, operation, field_names):
        ...
        if not self.env.user.has_group("hr.group_hr_employee_manager"):
    ...
    def _has_field_access(self, field, operation):
        ...
        return super()._has_field_access(field, operation) and (
            self.env.su
            or self.env.user.has_group("hr.group_hr_employee_manager")
    ...
    def get_formview_id(self, access_uid=None):
        ...
        if user.has_group('hr.group_hr_employee_manager'):
    ...
    def get_formview_action(self, access_uid=None):
        ...
        if not user.has_group('hr.group_hr_employee_manager'):
