    ...
    def _set_lot_ids(self):
        for move in self:
            if move.state == 'assigned' and all(ml.lot_id for ml in move.move_line_ids):
                # see test_lot_quantity_remains_unchanged_after_done
                # -> https://github.com/odoo/odoo/pull/91769
                # see test_tracked_and_manufactured_component (all)
                continue
            # faut checker la repartition des quantites des lots: p.ex qty=100, lot_ids=(lot1|lot2)... ?
            move_lines_commands = []
    ...
    def _set_quantity_done_prepare_vals(self, qty):
        ...
        for ml in self.move_line_ids:
            ...
            if self.product_uom.is_zero(qty):
                res.append((2, ml.id))
                continue
            if (c := ml.product_uom_id.compare(ml_qty, 0)) < 0:
                continue
            elif c == 0:
