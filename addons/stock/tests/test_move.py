...
class TestStockMove(TestStockCommon):
    ...
    def test_scrap_1(self):
        ...
        """
        self.env['stock.quant']._update_available_quantity(self.productA, self.stock_location, 1)
        scrap_form = Form(self.env['stock.move.line'])
        scrap_form.product_id = self.productA
        scrap_form.location_id = self.stock_location
        scrap_form.scrap_location_id = self.scrap_location
        scrap_form.quantity = 1
        scrap = scrap_form.save()
        scrap.do_scrap()
        move = self.env['stock.move'].search([('product_id', '=', self.productA.id), ('scrapped', '=', True)])
        self.assertEqual(move.state, 'done')
    ...
    def test_scrap_2(self):
        ...
        scrap.do_scrap()
        move = self.env['stock.move'].search([('product_id', '=', self.product_consu.id), ('scrapped', '=', True)])
        self.assertEqual(move.state, 'done')
        self.assertEqual(move.quantity, 1)
    ...
    def test_scrap_4(self):
        ...
        self.assertEqual(scrap.quantity, 8, 'Scrap quantity is not updated.')
    ...
    def test_scrap_6(self):
        ...
        warning_message = scrap.do_scrap()
        ...
        self.assertEqual(move.quantity, 1)
        self.assertEqual(move.product_uom_id, self.uom_dozen)
        self.assertEqual(move.state, 'done')
    ...
    def test_scrap_12_qty_in_sublocation(self):
        ...
        with Form(self.env['stock.move.line']) as scrap_form:
            scrap_form.product_id = self.productA
