...
class AccountEdiXmlUblTr(models.AbstractModel):
    ...
    def _get_tax_category_list(self, customer, supplier, taxes):
        ...
        for tax in taxes.filtered(lambda t: t.l10n_tr_tax_withholding_code_id):
            ...
            tax_scheme_name = tax.l10n_tr_tax_withholding_code_id.with_context(
                lang="tr_TR"
            ).name
            res.append(
                {
    ...
    def _tr_tax_totals(self, invoice, taxes_vals, withholding):
        ...
        def get_withholding_taxable_amount():
            return sum(
                v["tax_amount"]
                for k, v in taxes_vals["tax_details"].items()
                if not filter_tax_details(k)
            )
                ...
                tax_totals_vals["tax_subtotal_vals"].append(
                    {
                        ...
                        "taxable_amount": get_withholding_taxable_amount()
                        if withholding
                        else vals.get("base_amount_currency"),
                        "tax_amount": abs(vals.get("tax_amount_currency")),
                        "tax_category_vals": tax_category_vals,
                        "percent": subtotal_percent
                        if withholding
                        else tax_category_vals.get("percent", False),
                    }
                )
