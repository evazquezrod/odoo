...
class HrExpense(models.Model):
    ...
    def _compute_price_unit(self):
        """
           ...
           Otherwise it is always computed from the total_amount and the quantity else it would break the Ticket Entry
    ...
    def action_post(self):
        ...
        Post the expense, following one of those two options:
            ...
            - Employee-paid expenses: Through a wizard, create and post a ticket
        """
    ...
    def _post_without_wizard(self):
        ...
        for company, expenses in employee_expenses.grouped('company_id').items():
            ...
            expense_ticket_vals_list = [
                {
                    **new_ticket_vals,
                    'journal_id': journal.id,
                    'invoice_date': today,
                }
                for new_ticket_vals in expenses._prepare_tickets_vals()
            ]
            moves = self.env['account.move'].sudo().create(expense_ticket_vals_list)
            for move in moves:
    ...
    def _prepare_tickets_vals(self):
        attachments_data = []
