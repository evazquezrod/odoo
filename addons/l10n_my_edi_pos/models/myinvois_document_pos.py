...
class MyInvoisDocumentPoS(models.Model):
    ...
    def _split_consolidated_invoice_record_in_lines(self):
        ...
        sorted_order = self.pos_order_ids.sorted(reverse=True)
        ...
        orders_per_line = []
        for orders in all_orders_per_config.values():
            for order in orders:
                if continuous_orders and order not in self.pos_order_ids:
                    orders_per_line.append(continuous_orders)
                    continuous_orders = self.env['pos.order']
                elif order in self.pos_order_ids:
