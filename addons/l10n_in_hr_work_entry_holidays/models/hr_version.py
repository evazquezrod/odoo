...
class HrVersion(models.Model):
    ...
    def _get_version_work_entries_values(self, date_start, date_stop):
        ...
        public_holidays = self.env['resource.calendar.leaves'].search([
            ('resource_id', '=', False),
            ('company_id', 'in', in_contracts.company_id.ids),
            ('calendar_id', 'in', in_contracts.resource_calendar_id.ids + [False]),
        ])
        ...
        for contract in in_contracts:
            ...
            contract_calendar_id = contract.resource_calendar_id
            resource = employee.resource_id
            tz = pytz.timezone(contract_calendar_id.tz)

            attendance_intervals = list(contract_calendar_id._attendance_intervals_batch(
            ...
            for leave in leaves_by_employee[employee]:
                ...
                leave_start_dt, leave_end_dt = self._adjust_leave_dates(leave, start_dt, end_dt, tz, contract_calendar_id)
                filtered_public_holidays = public_holidays.filtered(lambda public_holiday:
                    ...
                    (not public_holiday.calendar_id or public_holiday.calendar_id == contract_calendar_id) and
                ...
                work_entry_values.extend(
                    {
                        ...
                        'date_stop': work_entry_start + timedelta(hours=contract_calendar_id.hours_per_day),
                    ...
                    if (
                        ...
                        work_entry_start + timedelta(hours=contract_calendar_id.hours_per_day)
    ...
    def _adjust_leave_dates(self, leave, start_dt, end_dt, tz, contract_calendar_id):
        ...
        elif self._is_non_working_day(contract_calendar_id, leave_start_dt) and self._is_non_working_day(contract_calendar_id, leave_end_dt):
            leave_start_dt = self._get_working_day(contract_calendar_id, leave_start_dt, 1)
            leave_end_dt = self._get_working_day(contract_calendar_id, leave_end_dt, -1)
